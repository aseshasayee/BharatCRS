...
export function mapComplaint(c) {
  return {
    ...
    title: c.normalized_input?.issue_summary || c.normalized_input?.raw_text || c.common_metadata?.raw_text || '',
    description: c.normalized_input?.raw_text || c.common_metadata?.raw_text || '',
    ...
    department: c.domain_classification?.assigned_department || c.governance_and_sla?.assigned_department || '',
    ward: c.spatio_temporal_core?.administrative_unit?.ward_id ? `Ward ${c.spatio_temporal_core.administrative_unit.ward_id}` : '',
    ward_id: c.spatio_temporal_core?.administrative_unit?.ward_id || '',
    lat: c.spatio_temporal_core?.location?.latitude,
    lon: c.spatio_temporal_core?.location?.longitude,
